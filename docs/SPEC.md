...
A portable Agent Skill repository containing:

1. **A standard skill** — `SKILL.md` plus bundled scripts an agent can run.
1. **A local runtime** — Python code under `scripts/lib/agentfeeds_runtime`.
1. **Agent-facing wrappers** — scripts for setup, management, refresh, and optional polling.
1. **An interop spec** — this document, plus the schemas it references.

Users install the skill or an agent integration that points at it. Their agent uses bundled scripts to discover templates, subscribe to streams, refresh data, and read stream data before falling back to web search.
...
├── SKILL.md                         # standard Agent Skill entrypoint
├── LICENSE                          # MIT
├── docs/
...
The primary agent-facing active stream map is the `python scripts/agentfeeds.py streams` command group. It keeps agents from depending on storage paths during normal operation:

```
python scripts/agentfeeds.py streams list --json
python scripts/agentfeeds.py streams search <topic> --json
python scripts/agentfeeds.py streams show <subscription-id> --json
python scripts/agentfeeds.py streams read <subscription-id> --json
```
...
`~/.agentfeeds/subscriptions.yaml` is the durable record. The fetcher reads it; agents update it through `python scripts/agentfeeds.py subscribe` and `python scripts/agentfeeds.py unsubscribe`.
...
When subscribing to a template with parameters, materialize it into a concrete instance. `python scripts/agentfeeds.py subscribe` may derive the `id` and `title`, or the user can pass `--id` and `--title`. Parameterless templates can use the template id/title as the concrete instance because there is only one natural instance.
...
`agentfeeds-catalog/catalog/INDEX.json` is auto-generated by the catalog repo's `scripts/build-index.py`. It is lightweight and inspectable, but agents should normally search it through `python scripts/agentfeeds.py templates search`.
...
## 8. The Fetcher (`python scripts/agentfeeds_fetch.py`)
...
```
python scripts/agentfeeds_fetch.py                        # default: refresh stale subscriptions
python scripts/agentfeeds_fetch.py --all                  # refresh every subscription regardless of staleness
python scripts/agentfeeds_fetch.py --stream <id>          # refresh one specific subscription id
python scripts/agentfeeds_fetch.py --once <id>            # one-shot fetch, used by `subscribe` recipe for eager first-fetch
python scripts/agentfeeds_fetch.py --regenerate-catalog   # regenerate ~/.agentfeeds/catalog.md without polling
python scripts/agentfeeds_fetch.py --update-catalog       # refresh ~/.agentfeeds/catalog-cache/ from public catalog
```
...
The primary user experience is agent-orchestrated. The user asks for outcomes in natural language; the agent uses `python scripts/agentfeeds.py` and `python scripts/agentfeeds_fetch.py` as an internal control plane, then reports results. The CLI exists for agents, scripts, and debugging, not as the normal operator workflow.
...
1. **At session start or when local context is relevant:** Use `python scripts/agentfeeds.py streams list` or `python scripts/agentfeeds.py streams search <topic>` to inspect active streams.
1. **When user asks about a topic covered by a subscribed stream:** Use `python scripts/agentfeeds.py streams read <subscription-id> --json`. Do not web-search if a non-stale stream covers the question.
1. **When the user asks to subscribe to something:** Search templates with `python scripts/agentfeeds.py templates search <query>`, inspect candidates with `python scripts/agentfeeds.py templates show <template-id>`, then subscribe.
1. **When state appears stale and the user asks about it:** Run `python scripts/agentfeeds_fetch.py --stream <subscription-id>` to refresh, then re-read.
1. **When the user asks what’s available:** Use `python scripts/agentfeeds.py templates search <query>` or `python scripts/agentfeeds.py templates list`.
...
1. Search templates with `python scripts/agentfeeds.py templates search <query>`.
...
1. Run `python scripts/agentfeeds.py subscribe <template-id> [key=value ...]`.
1. Run `python scripts/agentfeeds_fetch.py --once <subscription-id>` for eager first-fetch.
1. Confirm to user with `python scripts/agentfeeds.py streams read <subscription-id> --json`.

**`recipes/unsubscribe.md`** — run `python scripts/agentfeeds.py unsubscribe <subscription-id>`.

**`recipes/refresh.md`** — run `python scripts/agentfeeds_fetch.py --stream <subscription-id>` or `python scripts/agentfeeds_fetch.py --all`.

**`recipes/discover.md`** — run `python scripts/agentfeeds.py templates search`, present matches to the user, suggest which to subscribe to.
...
**`recipes/template-testing.md`** — run `python scripts/agentfeeds.py templates validate`, `python scripts/agentfeeds.py templates test`, confirm discovery, and smoke-test subscription materialization with a temporary Agent Feeds root before subscribing in the live root.
...
## 10. Polling Installer (`python scripts/polling/install.py`)
...
1. Install a cron entry / launchd plist that runs the installed `agentfeeds-fetch` runtime entrypoint at that interval.
1. Print confirmation and how to uninstall.

Companion script `python scripts/polling/uninstall.py` cleanly removes the cron/launchd entry.
...
- [ ] `python scripts/agentfeeds_fetch.py` polls the 8 starter streams and writes valid state files.
- [ ] `python scripts/agentfeeds.py streams list/show/read` reflects refreshed state after each fetch.
...
- [ ] At least one full end-to-end demo works: cold install → `subscribe me to weather in San Jose` → stream data exists → user asks “what’s the weather?” → agent reads via `python scripts/agentfeeds.py streams read` (no web search) → correct answer.
...
- **State files are fetcher-owned.** Agents should use `python scripts/agentfeeds.py streams read`; never write to `state/` from a recipe.
- **Subscription changes go through the CLI.** Use `python scripts/agentfeeds.py subscribe` and `python scripts/agentfeeds.py unsubscribe`.
