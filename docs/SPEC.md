...
`agentfeeds-catalog/catalog/INDEX.json` is auto-generated by the catalog repo's `scripts/build-index.py`. It is lightweight and inspectable, but agents should normally search it through `python3 scripts/agentfeeds.py templates search`.
...
1. **When the user asks what’s available:** Use `python3 scripts/agentfeeds.py templates search <query>` or `python3 scripts/agentfeeds.py templates list`.
...
1. Search templates with `python3 scripts/agentfeeds.py templates search <query>`.
...
1. Run `python3 scripts/agentfeeds_fetch.py --once <subscription-id>` for eager first-fetch.
...
**`recipes/refresh.md`** — run `python3 scripts/agentfeeds_fetch.py --stream <subscription-id>` or `python3 scripts/agentfeeds_fetch.py --all`.

**`recipes/discover.md`** — run `python3 scripts/agentfeeds.py templates search`, present matches to the user, suggest which to subscribe to.
...
**`recipes/template-testing.md`** — run `python3 scripts/agentfeeds.py templates validate`, `python3 scripts/agentfeeds.py templates test`, confirm discovery, and smoke-test subscription materialization with a temporary Agent Feeds root before subscribing in the live root.
...
Use `python3 scripts/agentfeeds.py polling uninstall` only when the user no longer wants ambient refresh.
