...
The primary agent-facing active stream map is the `python3 scripts/agentfeeds.py streams` command group. It keeps agents from depending on storage paths during normal operation:

```
python3 scripts/agentfeeds.py streams list --json
python3 scripts/agentfeeds.py streams find <topic> --json
...
The default brief MUST avoid volatile timestamps and freshness fields. Implementations MAY expose an explicit freshness/debug mode, but integrations SHOULD NOT use volatile brief output for prompt-cached session-start context.
...
`~/.agentfeeds/subscriptions.yaml` is the durable record. The fetcher reads it; agents update it through `python3 scripts/agentfeeds.py subscribe` and `python3 scripts/agentfeeds.py unsubscribe`.
...
When subscribing to a template with parameters, materialize it into a concrete instance. `python3 scripts/agentfeeds.py subscribe` may derive the `id` and `title`, or the user can pass `--id` and `--title`. Parameterless templates can use the template id/title as the concrete instance because there is only one natural instance.
...
`agentfeeds-catalog/catalog/INDEX.json` is auto-generated by the catalog repo's `scripts/build-index.py`. It is lightweight and inspectable, but agents should normally search it through `python3 scripts/agentfeeds.py templates find`.
...
1. **When checking ambient readiness:** Use `python3 scripts/agentfeeds.py admin polling status --json` and `python3 scripts/agentfeeds.py streams health --json`.
...
1. **When the user asks to subscribe to something:** Search templates with `python3 scripts/agentfeeds.py templates find <query>`, inspect candidates with `python3 scripts/agentfeeds.py templates show <template-id>`, preview with `subscribe --dry-run --json`, then subscribe.
1. **When state appears stale and the user asks about it:** Run `python3 scripts/agentfeeds.py refresh --stream <subscription-id>` to refresh, then re-read.
1. **When the user asks what’s available:** Use `python3 scripts/agentfeeds.py templates find <query>` or `python3 scripts/agentfeeds.py admin templates list`.
...
1. Search templates with `python3 scripts/agentfeeds.py templates find <query>`.
...
1. Run `python3 scripts/agentfeeds.py subscribe <template-id> [key=value ...] --dry-run --json`.
1. Run `python3 scripts/agentfeeds.py subscribe <template-id> [key=value ...]`.
1. Confirm to user with `python3 scripts/agentfeeds.py streams read <subscription-id> --json`.

**`recipes/unsubscribe.md`** — run `python3 scripts/agentfeeds.py unsubscribe <subscription-id>`.

**`recipes/refresh.md`** — run `python3 scripts/agentfeeds.py refresh --stream <subscription-id>` or `python3 scripts/agentfeeds.py refresh --all`.

**`recipes/discover.md`** — run `python3 scripts/agentfeeds.py templates find`, present matches to the user, suggest which to subscribe to.
...
**`recipes/template-testing.md`** — run `python3 scripts/agentfeeds.py admin templates validate`, `python3 scripts/agentfeeds.py admin templates test`, confirm discovery, and smoke-test subscription materialization with a temporary Agent Feeds root before subscribing in the live root.
...
Background refresh is required for normal ambient-context behavior. Agents should try to verify or install polling at session start:
...
Use `python3 scripts/agentfeeds.py admin polling uninstall` only when the user no longer wants ambient refresh.

If the scheduler is unsupported or unavailable, the system can still refresh explicitly, but ambient awareness is degraded because state may not stay warm between prompts.
...
- **State files are fetcher-owned.** Agents should use `python3 scripts/agentfeeds.py streams read`; never write to `state/` from a recipe.
- **Subscription changes go through the CLI.** Use `python3 scripts/agentfeeds.py subscribe` and `python3 scripts/agentfeeds.py unsubscribe`.
