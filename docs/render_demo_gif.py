...
def line_color(line: str) -> str:
    ...
    if stripped.startswith("-"):
        return BLUE
    if "python3 scripts/agentfeeds.py streams read" in line or "before web search" in line:
        return PINK
...
SCENES = [
    ...
    (
        "Use case: calendar awareness without repeated setup",
        "The user asks naturally; the agent sees a matching active stream.",
        "User: What is on my calendar today?\n\nAgent: I see a fresh Calendar.app agenda stream. I'll run python3 scripts/agentfeeds.py streams read first instead of rebuilding the calendar context.\n\nAgent: Here is today's agenda from local Agent Feeds state.",
        "Fresh answers from local state.",
        4300,
    ),
    (
        "Use case: personal task awareness",
        "Streams can cover private/local state, not just public feeds.",
        "User: What reminders are still open?\n\nAgent: There is a mac/reminders-pending stream. I'll inspect it with python3 scripts/agentfeeds.py streams read.\n\nAgent: Here are the open reminders, grouped by list and due date.",
        "The agent can use local/private sources conversationally.",
        4300,
    ),
    (
        "Use case: project notes and weather snapshots",
        "Snapshot streams answer quick factual questions without repeated setup.",
        "User: Refresh Project notes and summarize them with Santa Clara weather.\n\nAgent: I see subscribed local file and weather streams. I'll refresh the notes stream, read both snapshots, and summarize them together.\n\nAgent: Answer is grounded in timestamped Agent Feeds state, not chat memory.",
